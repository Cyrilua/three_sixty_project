...
q.attr("data-step");if("2"!==p&&"3"!==p)throw Error("Unexpected attribute on search");if("2"===p){var n=$("input[name=participants]:checked").attr("data-participant-id");n={pollId:t,csrfmiddlewaretoken:u,checkedTarget:n}}else"3"===p&&($("input[name=participants]:checked").each(function(v,x){checked.push($(x).attr("data-participant-id"))}),n={pollId:t,csrfmiddlewaretoken:u,checkedInterviewed:[]});$.ajax({url:"step/"+p+"/category/"+d+"/",type:"post",data:n,beforeSend:function(){clearTimeout(K);c.addClass("disabled");
...
function(d){l(d)});r.on("click",".team__action",function(){var d=$(this).parent().parent().parent(),b=d.attr("data-team-id");$(".team--selected").each(function(a,c){a=$(c).attr("data-team-id");b!==a&&$(c).toggleClass("team--selected")});d.toggleClass("team--selected")});var C;r.on("input",".input__search",function(d){var b=$(".sort"),a=$(".content"),c=$(".loader-round"),h=c.children(".loader__status"),p=q.attr("data-step"),n=$(".input__search").val();if("2"!==p&&"3"!==p)throw Error("Unexpected attribute on search");
if("2"===p)var v={input:n,pollId:t,csrfmiddlewaretoken:u,mode:$(d.target).attr("data-mode"),checkedTarget:$("input[name=participants]:checked").attr("data-participant-id")};else"3"===p&&($("input[name=participants]:checked").each(function(x,E){checked.push($(E).attr("data-participant-id"))}),v={input:n,pollId:t,csrfmiddlewaretoken:u,mode:$(d.target).attr("data-mode"),checkedInterviewed:[]});C=$.ajax({url:"step/"+p+"/search/",type:"post",data:v,beforeSend:function(){C&&C.abort();c.removeClass("hide");
h.removeClass("status--loading status--done status--error").addClass("status--loading");b.addClass("disabled");a.empty()},complete:function(){C=void 0;b.removeClass("disabled");c.addClass("hide");h.removeClass("status--loading status--done status--error")},success:function(x){a[0].insertAdjacentHTML("afterbegin",x.content)}})});r.on("click","[name=participants]",function(){0<$("input[name=participants]:checked").length?$("#sendPoll").prop({disabled:!1}):$("#sendPoll").prop({disabled:!0})});r.on("click",
"#backToStep2",function(d){m(d)});r.on("click","#sendPoll",function(d){var b=[];$("input[name=participants]:checked").each(function(a,c){b.push($(c).attr("data-participant-id"))});$.ajax({url:"send/",type:"post",data:{pollId:t,csrfmiddlewaretoken:u,checkedInterviewed:b},beforeSend:function(){f.addClass("disabled");q.addClass("disabled")},success:function(){location.href="polls/"},error:function(){q.removeClass("disabled")}})});r.on("click",".item__link",function(d){d=$(this).parent();var b=d.attr("data-step"),
a=q.attr("data-step");console.log(a,b);if(!d.hasClass("item--active")&&b!==a)if("1"===a&&"2"===b)g();else if("1"===a&&"3"===b)A();else if("2"===a&&"1"===b)k();else if("2"===a&&"3"===b)l();else if("3"===a&&"1"===b)y();else if("3"===a&&"2"===b)m();else throw Error("Unexpected attribute when going to another step");})});
