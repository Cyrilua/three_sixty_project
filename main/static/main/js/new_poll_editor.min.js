...
$(function(){function e(d){var b=D(),a=$(d).parent().children(".save__status");console.log(b);$.ajax({url:"save_as/",type:"post",data:{csrfmiddlewaretoken:v,pollId:t,template:b},beforeSend:function(){f.addClass("disabled");q.addClass("disabled");a.removeClass("status--loading status--error status--done").addClass("status--loading")},success:function(){Snackbar.show({text:"\u0428\u0430\u0431\u043b\u043e\u043d \u0443\u0441\u043f\u0435\u0448\u043d\u043e \u0441\u043e\u0437\u0434\u0430\u043d",showAction:!0,
...
f.eq(1).addClass("item--active")},complete:function(){q.removeClass("disabled");f.eq(1).removeClass("disabled");f.eq(0).removeClass("disabled");w&&f.eq(2).removeClass("disabled")},error:function(){}})}function k(d){d=$("input[name=participants]:checked").attr("data-participant-id");$.ajax({url:"step/1/from/2/",type:"post",data:{csrfmiddlewaretoken:v,checkedTarget:d,pollId:t},beforeSend:function(){f.addClass("disabled");q.addClass("disabled")},success:function(b){var a=$(".head__main");a.empty();a[0].insertAdjacentHTML("afterbegin",
...
complete:function(){f.eq(0).removeClass("disabled");f.eq(1).removeClass("disabled");w&&f.eq(2).removeClass("disabled");q.removeClass("disabled")}})}function A(d){d=D();console.log(d);$.ajax({url:"step/3/from/1/",type:"post",data:{csrfmiddlewaretoken:v,pollId:t,template:d},beforeSend:function(){q.addClass("disabled");f.addClass("disabled")},success:function(b){document.documentElement.style.setProperty("--mdc-theme-primary","#FF1841");document.documentElement.style.setProperty("--mdc-theme-secondary",
...
a.attr({"data-question-type":p});if("radio"===h&&"checkbox"===p||"checkbox"===h&&"radio"===p)$(n.children(".answer")).each(function(x,E){x=$(E).children(".answer__icon");x.empty();x[0].insertAdjacentHTML("afterbegin",u)});else if(n.empty(),"radio"===p||"checkbox"===p)c=document.createElement("div"),c.classList.add("answer"),n.append(c),a=document.createElement("div"),a.classList.add("answer__icon"),c.append(a),a.insertAdjacentHTML("afterbegin",u),a=document.createElement("textarea"),a.classList.add("textarea-line",
...
p=$(c).children(".question__answers");b.questions.push({id:$(c).attr("data-question-id"),serialNumber:a+1,type:h,name:$(c).children(".question__main").children(".question__name ").val()});if("radio"===h||"checkbox"===h)c=p.children(".answer"),b.questions[a].answers=[],b.questions[a].countAnswers=c.length,$(c).each(function(n,u){n=$(u).children(".answer__text").val();b.questions[a].answers.push({text:n,id:u+1})});else if("range"===h)c=p.children(".slider-range").children(".slider-range__settings"),
...
q.attr("data-step");if("2"===p||"3"===p){$(".category").removeClass("active-sort");$(this).addClass("active-sort");if("2"===p){var n=$("input[name=participants]:checked").attr("data-participant-id");n={pollId:t,csrfmiddlewaretoken:v,checkedTarget:n}}else"3"===p&&($("input[name=participants]:checked").each(function(u,x){checked.push($(x).attr("data-participant-id"))}),n={pollId:t,csrfmiddlewaretoken:v,checkedInterviewed:[]});$.ajax({url:"step/"+p+"/category/"+d+"/",type:"post",data:n,beforeSend:function(){clearTimeout(K);
...
c){$(c).css({order:"initial"})}),b.css({order:-1E4})):($(".participant-active").each(function(a,c){$(c).css({order:"initial","background-color":"#FAFAFA"})}),b.css({order:-1E4,"background-color":"#F0F1F6"}));window.scrollTo(0,d);b.addClass("participant-active");$("#nextToStep3").prop({disabled:!1});w=!0;f.eq(2).addClass("disabled")});r.on("click","#nextToStep3",function(d){l(d)});r.on("click",".team__action",function(){var d=$(this).parent().parent().parent(),b=d.attr("data-team-id");$(".team--selected").each(function(a,
c){a=$(c).attr("data-team-id");b!==a&&$(c).toggleClass("team--selected")});d.toggleClass("team--selected");d.children(".team__up")[0].scrollIntoView(!1)});var C;r.on("input",".input__search",function(d){var b=$(".sort"),a=$(".content"),c=$(".loader-round"),h=c.children(".loader__status"),p=q.attr("data-step"),n=$(".input__search").val();if("2"!==p&&"3"!==p)throw Error("Unexpected attribute on search");if("2"===p)var u={input:n,pollId:t,csrfmiddlewaretoken:v,mode:$(d.target).attr("data-mode"),checkedTarget:$("input[name=participants]:checked").attr("data-participant-id")};
else"3"===p&&($("input[name=participants]:checked").each(function(x,E){checked.push($(E).attr("data-participant-id"))}),u={input:n,pollId:t,csrfmiddlewaretoken:v,mode:$(d.target).attr("data-mode"),checkedInterviewed:[]});C=$.ajax({url:"step/"+p+"/search/",type:"post",data:u,beforeSend:function(){C&&C.abort();c.removeClass("hide");h.removeClass("status--loading status--done status--error").addClass("status--loading");b.addClass("disabled");a.empty()},complete:function(){C=void 0;b.removeClass("disabled");
c.addClass("hide");h.removeClass("status--loading status--done status--error")},success:function(x){a[0].insertAdjacentHTML("afterbegin",x.content)}})});r.on("click","[name=participants]",function(){0<$("input[name=participants]:checked").length?$("#sendPoll").prop({disabled:!1}):$("#sendPoll").prop({disabled:!0})});r.on("click","#backToStep2",function(d){m(d)});r.on("click","#sendPoll",function(d){var b=[];$("input[name=participants]:checked").each(function(a,c){b.push($(c).attr("data-participant-id"))});
$.ajax({url:"send/",type:"post",data:{pollId:t,csrfmiddlewaretoken:v,checkedInterviewed:b},beforeSend:function(){f.addClass("disabled");q.addClass("disabled")},success:function(){location.href="polls/"},error:function(){q.removeClass("disabled")}})});r.on("click",".item__link",function(d){d=$(this).parent();var b=d.attr("data-step"),a=q.attr("data-step");if(!d.hasClass("item--active")&&b!==a)if("1"===a&&"2"===b)g();else if("1"===a&&"3"===b)A();else if("2"===a&&"1"===b)k();else if("2"===a&&"3"===b)l();
