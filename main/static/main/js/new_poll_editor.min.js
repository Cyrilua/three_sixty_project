...
b.headMain);a=$(".head__move");a.empty();a[0].insertAdjacentHTML("afterbegin",b.headMove);a=$(".categories-block");a.empty();a[0].insertAdjacentHTML("afterbegin",b.categories);q.attr({"data-step":"1"});f.eq(1).removeClass("item--active");f.eq(0).addClass("item--active");E()},complete:function(){f.eq(0).removeClass("disabled");f.eq(1).removeClass("disabled");w&&f.eq(2).removeClass("disabled");q.removeClass("disabled")}})}function l(d){d=$("input[name=participants]:checked").attr("data-participant-id");
...
c.empty();c[0].insertAdjacentHTML("afterbegin",a.headMove);c=$(".categories-block");c.empty();c[0].insertAdjacentHTML("afterbegin",a.categories);q.attr({"data-step":"2"});f.eq(2).removeClass("item--active");f.eq(1).addClass("item--active")},complete:function(){f.eq(0).removeClass("disabled");f.eq(1).removeClass("disabled");w&&f.eq(2).removeClass("disabled");q.removeClass("disabled")}})}function x(d){var b=[];$("input[name=participants]:checked").each(function(a,c){b.push($(c).attr("data-participant-id"))});
...
"black"));$(".textarea-line").each(function(c,h){""!==h.value&&B(h,5)});$(".textarea-border").each(function(c,h){""!==h.value&&B(h,-1)});d.each(function(c,h){F(h);"range"===$(h).attr("data-question-type")&&G(h);z.push($(h).attr("data-question-id"))})}function K(d){var b=document.createElement("div");b.classList.add("answer");var a=document.createElement("div");a.classList.add("answer__icon");b.append(a);if("radio"===d)d='<div class="mdc-form-field">\n                            <div class="mdc-radio mdc-radio--disabled">\n                                <input class="mdc-radio__native-control" type="radio" id="" name="" disabled>\n                                <div class="mdc-radio__background">\n                                    <div class="mdc-radio__outer-circle"></div>\n                                    <div class="mdc-radio__inner-circle"></div>\n                                </div>\n                                <div class="mdc-radio__ripple"></div>\n                            </div>\n                        </div>';
...
else throw Error("Unexpected attribute when adding answer option");a.insertAdjacentHTML("afterbegin",d);a=document.createElement("textarea");a.classList.add("textarea-line","answer__text");$(a).attr({name:"",id:"",rows:"1",placeholder:"\u0412\u0430\u0440\u0438\u0430\u043d\u0442 \u043e\u0442\u0432\u0435\u0442\u0430",maxlength:150});b.append(a);a=document.createElement("img");a.classList.add("answer__clear");$(a).attr({src:"/static/main/images/icon/clear-24px.svg",alt:""});b.append(a);return b}function F(d){var b=
...
a.attr({"data-question-type":p});if("radio"===h&&"checkbox"===p||"checkbox"===h&&"radio"===p)$(n.children(".answer")).each(function(y,L){y=$(L).children(".answer__icon");y.empty();y[0].insertAdjacentHTML("afterbegin",v)});else if(n.empty(),"radio"===p||"checkbox"===p)c=document.createElement("div"),c.classList.add("answer"),n.append(c),a=document.createElement("div"),a.classList.add("answer__icon"),c.append(a),a.insertAdjacentHTML("afterbegin",v),a=document.createElement("textarea"),a.classList.add("textarea-line",
...
n.textContent="\u0414\u043e",a.append(n),n=document.createElement("input"),n.classList.add("input-transparent","slider-range__max"),$(n).attr({type:"number",id:"max_"+c,min:"1",max:"100",value:"10"}),a.append(n),G(d);else throw Error("Unexpected attribute on question change");}})}function G(d){var b=new mdc.slider.MDCSlider(d.querySelector(".mdc-slider"));b.min=parseInt($(d).find(".slider-range__min").val());b.max=parseInt($(d).find(".slider-range__max").val());b.step=parseInt($(d).find(".slider-range__step").val());
b.listen("MDCSlider:change",function(){console.log("Value changed to "+b.value)})}function H(d){for(var b in z)if(b===d){d=H(I());break}return d}function I(){for(var d="",b=0;33>b;b++)d+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random()));return d}function D(){var d=$(".question"),b={pollId:t,name:$(".poll__name").val(),description:$(".poll__description").val(),questions:[],color:$(".color__variable--select").attr("data-color"),countQuestion:d.length};
...
F(b);c=document.createElement("div");c.classList.add("question__answers");b.append(c);a=document.createElement("div");a.classList.add("answer");c.append(a);c=document.createElement("div");c.classList.add("answer__icon");a.append(c);c.insertAdjacentHTML("afterbegin",'<div class="mdc-form-field">\n                            <div class="mdc-radio mdc-radio--disabled">\n                                <input class="mdc-radio__native-control" type="radio" id="" name="" disabled>\n                                <div class="mdc-radio__background">\n                                    <div class="mdc-radio__outer-circle"></div>\n                                    <div class="mdc-radio__inner-circle"></div>\n                                </div>\n                                <div class="mdc-radio__ripple"></div>\n                            </div>\n                        </div>');
...
r.on("click",".new-answer__btn",function(){var d=$(this).parent(),b=d.attr("data-question-type");if("radio"===b||"checkbox"===b)d=d.children(".question__answers"),d.append(K(b)),15<=d.children(".answer").length&&$(this).addClass("hide");else throw Error("Unexpected attribute when adding answer option");});r.on("click",".answer__clear",function(){var d=$(this).parent(),b=d.parent(),a=b.parent(),c=a.attr("data-question-type");if("radio"===c||"checkbox"===c){b=b.children(".answer").length;if(15===b)a.children(".new-answer__btn").removeClass("hide");
else if(1===b){d.children(".answer__text").val("");return}d.remove()}else throw Error("Unexpected attribute when adding answer option");});r.on("click","#cancel",function(){location.href="/polls/"});r.on("click","#saveAs",function(){e(this)});r.on("click","#nextToStep2",function(d){g(d)});r.on("click","#backToStep1",function(d){k(d)});var J;r.on("click",".category",function(){var d=$(this).attr("data-part-url"),b=$(".loader-round"),a=$(".loader__status"),c=$(".substrate"),h=$(".input__search"),p=
q.attr("data-step");if("2"!==p&&"3"!==p)throw Error("Unexpected attribute on search");if("2"===p){var n=$("input[name=participants]:checked").attr("data-participant-id");n={pollId:t,csrfmiddlewaretoken:u,checkedTarget:n}}else"3"===p&&($("input[name=participants]:checked").each(function(v,y){checked.push($(y).attr("data-participant-id"))}),n={pollId:t,csrfmiddlewaretoken:u,checkedInterviewed:[]});$.ajax({url:"step/"+p+"/category/"+d+"/",type:"post",data:n,beforeSend:function(){clearTimeout(J);c.addClass("disabled");
b.removeClass("hide");a.removeClass("status--loading status--done status--error").addClass("status--loading");f.addClass("disabled");q.addClass("disabled")},success:function(v){if("participants"===d)h.attr({placeholder:"\u041f\u043e\u0438\u0441\u043a \u043f\u043e \u0443\u0447\u0430\u0441\u0442\u043d\u0438\u043a\u0430\u043c...","data-mode":"participants"});else if("teams"===d)h.attr({placeholder:"\u041f\u043e\u0438\u0441\u043a \u043f\u043e \u043a\u043e\u043c\u0430\u043d\u0434\u0430\u043c...","data-mode":"teams"});
else throw Error("unexpected attribute when changing category");h.prop({disabled:!1});var y=$(".content");y.empty();y[0].insertAdjacentHTML("afterbegin",v.content);a.removeClass("status--loading status--done status--error").addClass("status--done");0<$("input[name=participants]:checked").length?"2"===p?$("#nextToStep3").prop({disabled:!1}):"3"===p&&$("#sendPoll").prop({disabled:!1}):"2"===p?$("#nextToStep3").prop({disabled:!0}):"3"===p&&$("#sendPoll").prop({disabled:!0})},complete:function(){c.removeClass("disabled");
J=setTimeout(function(){a.removeClass("status--loading status--done status--error");b.addClass("hide")},2E3);f.eq(0).removeClass("disabled");f.eq(1).removeClass("disabled");w&&f.eq(2).removeClass("disabled");q.removeClass("disabled")},error:function(){h.prop({disabled:!0});a.removeClass("status--loading status--done status--error").addClass("status--error")}})});r.on("click","[name=participants]",function(){$("#nextToStep3").prop({disabled:!1});w=!0;f.eq(2).addClass("disabled")});r.on("click","#nextToStep3",
function(d){l(d)});r.on("click",".team__action",function(){var d=$(this).parent().parent().parent(),b=d.attr("data-team-id");$(".team--selected").each(function(a,c){a=$(c).attr("data-team-id");b!==a&&$(c).toggleClass("team--selected")});d.toggleClass("team--selected")});var C;r.on("input",".input__search",function(d){var b=$(".sort"),a=$(".content"),c=$(".loader-round"),h=c.children(".loader__status"),p=q.attr("data-step");if("2"!==p&&"3"!==p)throw Error("Unexpected attribute on search");if("2"===
p)var n={pollId:t,csrfmiddlewaretoken:u,mode:$(d.target).attr("data-mode"),checkedTarget:$("input[name=participants]:checked").attr("data-participant-id")};else"3"===p&&($("input[name=participants]:checked").each(function(v,y){checked.push($(y).attr("data-participant-id"))}),n={pollId:t,csrfmiddlewaretoken:u,mode:$(d.target).attr("data-mode"),checkedInterviewed:[]});C=$.ajax({url:"step/"+p+"/search/",type:"post",data:n,beforeSend:function(){C&&C.abort();c.removeClass("hide");h.removeClass("status--loading status--done status--error").addClass("status--loading");
...
b)A();else if("2"===a&&"1"===b)k();else if("2"===a&&"3"===b)l();else if("3"===a&&"1"===b)x();else if("3"===a&&"2"===b)m();else throw Error("Unexpected attribute when going to another step");})});
