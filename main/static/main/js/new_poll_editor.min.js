...
p=$(c).children(".question__answers");b.questions.push({id:$(c).attr("data-question-id"),serialNumber:a+1,type:h,name:$(c).children(".question__main").children(".question__name ").val()});if("radio"===h||"checkbox"===h)c=p.children(".answer"),b.questions[a].answers=[],b.questions[a].countAnswers=c.length,$(c).each(function(n,v){n=$(v).children(".answer__text").val();b.questions[a].answers.push(n)});else if("range"===h)c=p.children(".slider-range").children(".slider-range__settings"),b.questions[a].settingsSlider=
{min:c.children(".min").children(".slider-range__min").val(),max:c.children(".max").children(".slider-range__max").val(),step:c.children(".step").children(".slider-range__step").val()};else if("openQuestion"!==h)throw Error("unexpected attribute when receiving a poll");});return b}var r=$("body"),q=$(".editor "),f=$(".menu-r").children(".menu").children(".menu__item"),u=$('input[name="csrfmiddlewaretoken"]').val(),z=[],w=!1,t;F();r.on("click",".color__variable",function(){$(".color__variable").removeClass("color__variable--select");
...
c){a=$(c).attr("data-team-id");b!==a&&$(c).toggleClass("team--selected")});d.toggleClass("team--selected");d.children(".team__up")[0].scrollIntoView(!1)});var C;r.on("input",".input__search",function(d){var b=$(".sort"),a=$(".content"),c=$(".loader-round"),h=c.children(".loader__status"),p=q.attr("data-step"),n=$(".input__search").val();if("2"!==p&&"3"!==p)throw Error("Unexpected attribute on search");if("2"===p)var v={input:n,pollId:t,csrfmiddlewaretoken:u,mode:$(d.target).attr("data-mode"),checkedTarget:$("input[name=participants]:checked").attr("data-participant-id")};
else"3"===p&&($("input[name=participants]:checked").each(function(x,E){checked.push($(E).attr("data-participant-id"))}),v={input:n,pollId:t,csrfmiddlewaretoken:u,mode:$(d.target).attr("data-mode"),checkedInterviewed:[]});C=$.ajax({url:"step/"+p+"/search/",type:"post",data:v,beforeSend:function(){C&&C.abort();c.removeClass("hide");h.removeClass("status--loading status--done status--error").addClass("status--loading");b.addClass("disabled");a.empty()},complete:function(){C=void 0;b.removeClass("disabled");
